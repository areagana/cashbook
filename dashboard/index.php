...
if (isVerified()) 
{
        ...
        $coreSql = "SELECT
            ...
            $vars = [
                $book_id, // total cashin
                $book_id, // total cashout

                $book_id, // balance cashin
                $book_id, // balance cashout

                $book_id, // count cashins
                $book_id, // count cashouts

                $book_id, // today cashin
                $book_id, // today cashout

                $book_id, // month cashin
                $book_id, // month cashout

                $book_id, // BBF cashin
                $book_id  // BBF cashout
            ];

            $coreResult = prepared_statements($coreSql, $binds, $vars);
        ...
        $monthlyResult = prepared_statements(
            $monthlySql,
            'iiii',
            [
                $book_id,
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $cashin = (float)($monthlyMap[$m]['cashin'] ?? 0);
            $cashout = (float)($monthlyMap[$m]['cashout'] ?? 0);

            $monthlyCashin[] = $cashin;
            $monthlyCashout[] = $cashout;

            // Net movement for the month
            $monthlyBalance[] = $cashin - $cashout;
        }
...
?>
        ...
        <div class="container-fluid cb-dashboard">
            <div class="container-fluid px-lg-4">
                ...
                <div class="row g-3">
                    <div class="col-xl-8"><div class="cb-card p-3 h-100"><div class="d-flex justify-content-between align-items-center mb-3"><div><h5 class="section-title">Recent Transactions</h5><div class="section-sub">Latest recorded cash movements</div></div><a href="../books/?bkid=<?=$id_enc;?>" class="btn btn-sm btn-outline-secondary">View All</a></div><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Date</th><th>Details</th><th>Type</th><th class="text-end">Cash In</th><th class="text-end">Cash Out</th></tr></thead><tbody>
                    ...
                    <div class="col-xl-4"><div class="cb-card p-3 h-100"><div class="d-flex justify-content-between align-items-center mb-3"><div><h5 class="section-title">Customer Balances</h5><div class="section-sub">Outstanding balances from latest ledger positions</div></div><a href="../customers/?bsid=<?=$id_enc;?>" class="btn btn-sm btn-outline-secondary">View All</a></div><hr><?php if($topCustomers): $k=0; foreach($topCustomers as $customer): if($k >= 7) break;?><div class="d-flex justify-content-between align-items-center py-2 border-bottom"><div><strong><?=$customer['customer_name'];?></strong><div class="small text-muted">Current outstanding balance</div></div><strong class="amount-out"><?=number_format($customer['balance'],0);?>/=</strong></div><?php $k++; endforeach; else: ?><div class="empty-state">No outstanding customer balances found.</div><?php endif; ?></div></div>
                </div>
            </div>
        </div>
        <?php pageFooter(); ?>
